...
HTML_P3 = """
    ...
    <div id="tab-trangchu" class="tab active">
      ...
      <!-- API CONNECT URL CARD -->
      <div class="card" style="background:linear-gradient(160deg,rgba(0,200,255,0.07),rgba(10,16,28,0.95));border-color:rgba(0,200,255,0.35);">
        <div class="card-title"><i class="fa-solid fa-plug-circle-check" style="color:#10d98a;"></i> URL API - Kết Nối Ứng Dụng Bên Ngoài</div>
        <div style="font-size:0.76rem;color:var(--muted);margin-bottom:10px;line-height:1.6;">
          Copy URL này dán vào app/tool của bạn để kiểm tra key và đăng ký thiết bị:
        </div>
        <div id="apiUrlDisplay" style="background:rgba(0,0,0,0.55);border:1px solid rgba(0,200,255,0.3);border-radius:10px;padding:12px 14px;font-family:'Orbitron',sans-serif;font-size:0.66rem;color:#10d98a;word-break:break-all;letter-spacing:0.3px;line-height:1.7;"></div>
        <button onclick="copyApiUrl()" class="btn btn-primary" style="margin-top:10px;padding:11px;font-size:0.82rem;background:linear-gradient(135deg,#10d98a,#00c8ff);"><i class="fa-solid fa-copy"></i> COPY URL API</button>
        <div style="margin-top:13px;font-size:0.74rem;line-height:1.85;background:rgba(0,0,0,0.3);border-radius:9px;padding:11px 13px;border:1px solid rgba(255,255,255,0.05);">
          <div style="color:var(--blue);font-weight:800;margin-bottom:7px;font-size:0.76rem;"><i class="fa-solid fa-code"></i> CÁCH TÍCH HỢP VÀO APP/TOOL:</div>
          <div style="margin-bottom:4px;"><b style="color:var(--success);">GET:</b> <span style="color:var(--muted);">/api/check_key?key=KEY&amp;device_id=DEVICE_ID</span></div>
          <div style="margin-bottom:8px;"><b style="color:var(--purple);">POST:</b> <span style="color:var(--muted);">body: key=KEY&amp;device_id=DEVICE_ID</span></div>
          <div style="color:var(--warn);font-size:0.71rem;line-height:1.6;"><i class="fa-solid fa-circle-info"></i> Kết quả JSON trả về:<br>
          <span style="color:#10d98a;">{"status":"success","message":"Key hop le","time_left":"7 ngay"}</span><br>
          <span style="color:var(--danger);">{"status":"expired"} | {"status":"invalid"} | {"status":"device_limit"}</span></div>
        </div>
      </div>
      {% endif %}
    </div>
...
(function(){
  ...
  var el = document.getElementById('apiUrlDisplay');
  if(el) {
    el.innerHTML = '<b style="color:var(--blue);">Server URL:</b> ' + base + '<br><b style="color:var(--success);">Check Key:</b> ' + base + '/api/check_key';
  }
})();
function copyApiUrl(){
  var url = window.location.origin + '/api/check_key';
  navigator.clipboard.writeText(url).then(function(){
    alert('Đã copy URL API:\n' + url + '\n\nDùng với tham số: ?key=KEY&device_id=DEVICE_ID');
  }).catch(function(){ prompt('Copy URL này:', url); });
}
